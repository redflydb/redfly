...
// --------------------------------------------------------------------------------
REDfly.dialog.crmSegmentRecord = Ext.data.Record.create([
    ...
    { name: 'end' },
    { name: 'evidence_id' },
    { name: 'evidence_subtype_id', allowBlank: true, defaultValue: ''  },
    { name: 'evidence_subtype_term', allowBlank: true, defaultValue: ''  },
    { name: 'evidence_term' },
...
// --------------------------------------------------------------------------------
// The crmSegmentStore JsonStore is used to load and save CRM segments
// through the REST service.
// --------------------------------------------------------------------------------
REDfly.store.crmSegmentStore = new Ext.data.JsonStore({
    ...
    proxy: new Ext.data.HttpProxy({
        api: {
            create: {
                method: 'POST',
                url: REDfly.config.apiUrl + '/jsonstore/crmsegment/save'
            },
            // No destroy for this store but the spec can not be undefined!
            destroy: {
                url: REDfly.config.apiUrl + '/jsonstore/crmsegment/delete'
            },
            read: {
                method: 'GET',
                url: REDfly.config.apiUrl + '/jsonstore/crmsegment/load'
            },
            update: {
                method: 'POST',
                url: REDfly.config.apiUrl + '/jsonstore/crmsegment/save'
    ...
    fields: REDfly.dialog.crmSegmentRecord,
...
// --------------------------------------------------------------------------------
// Create the new/edit CRM segment dialog by extending Ext.FormPanel. This is
// used for CRUD operations on a CRM segment and contains functionality to
// automatically query citations and coordinates.
// --------------------------------------------------------------------------------
REDfly.component.crmSegmentPanel = Ext.extend(Ext.FormPanel, {
    ...
    formElements: {},
    store: REDfly.store.crmSegmentStore,
    userId: null,
    userFullName: null,
    initComponent: function() {
        ...
        this.data = new REDfly.dialog.crmSegmentRecord();
        ...
        var evidenceSubtypeItem = new REDfly.component.selectEvidenceSubtype();
        ...
        var curateAnatomicalExpressionTermsItem = new REDfly.component.curateCrmSegmentAnatomicalExpressionTerms();
        ...
        var curateStagingDataItem = new REDfly.component.curateCrmSegmentStagingData();
            ...
            buttonList = [
                {
                    ...
                    handler: function(button, event) {
                        // Update the data in the form and save it as long as the form is valid.
                        this.setAction('save');
                        this.save();
                    },
                    scope: this,
        ...
        Ext.apply(
            this,
            {
                ...
                formElements: {
                    ...
                    evidenceSubtype: evidenceSubtypeItem,
        ...
        REDfly.component.crmSegmentPanel.superclass.initComponent.apply(this, arguments);
        ...
        // Handle errors during the loading of the citation
        ...
        // Handle the loading of coordinates based on user input.
        ...
        // anyone from the buttons (but the Cancel button) at the bottom of the CRM segment
        // editing panel is pushed
        ...
        curateAnatomicalExpressionTermsItem.on(
            ...
            function(stagingData) {
                ...
                var newStagingDataRecord = new stagingDataRecord({
                    ...
                    crm_segment_id: stagingData.get('crm_segment_id'),
        ...
        curateStagingDataItem.on(
            ...
            function(recordIndex, stagingData) {
                ...
                existingStagingDataRecord.set('crm_segment_id', stagingData.get('crm_segment_id'));
    ...
    loadFormFromRecord: function(store, recordList, opt) {
        ...
        this.loadEvidenceSubtype(formRecord.get('evidence_subtype_id'));
        ...
        this.formElements.anatomicalExpressionTerms.setCrmsId(recordList[0].json.id);
    ...
    // Also updates the data record gene_id and gene_name.
    ...
    loadEvidence: function(evidenceId) {
        this.formElements.evidence.getStore().load({
            callback: function() {
    ...
    // --------------------------------------------------------------------------------
    // Load a single evidence term and set the selected evidence subtype.
    // Also updates the data record evidence_subtype_id and evidence_subtype_term.
    // @param {String} evidenceSubtypeId The evidence subtype id.
    // --------------------------------------------------------------------------------
    loadEvidenceSubtype: function(evidenceSubtypeId) {
        this.formElements.evidenceSubtype.getStore().load({
            callback: function() {
                this.formElements.evidenceSubtype.setValue(evidenceSubtypeId);
                var record = this.formElements.evidenceSubtype.getStore().getById(evidenceSubtypeId);
                this.data.set('evidence_subtype_id', record.data.id);
                this.data.set('evidence_subtype_term', record.data.term);
            },
            scope: this
        });
    },
    ...
    // @param {String} sequenceSourceId The sequence source id.
    // --------------------------------------------------------------------------------
    loadSequenceSource: function(sequenceSourceId) {
        this.formElements.sequenceSource.getStore().load({
            callback: function() {
    ...
    verifyLocalFormElements: function(errorList) {
        ...
        if ( this.formElements.isNegative.getValue() &&
            ...
            errorList.push('Negative CRM segments may not have anatomical expression terms.');
        }
    ...
    //         margin of other CRM segment(s), although it can be overrode by the
    //         curator.
    // @param errorList An array of error messages generated by this verification
    // @returns True on success and false if there was a verification error
    // --------------------------------------------------------------------------------
    verifyRemoteFormElements: function(config) {
        ...
        Ext.Ajax.request({
            ...
            url: REDfly.config.apiUrl + '/jsonstore/crmsegment/checkAnyDuplicateKind'
        });
    },
    // --------------------------------------------------------------------------------
    ...
    save: function(redflyIdList) {
        // When saving an CRM segment for approval we want to be able send the list of
        ...
        // contains a serialized list.  If this is not an approval be sure to set the
        // list to empty/null!
    ...
    // --------------------------------------------------------------------------------
    // Handle exceptions generated by the CRM segment store.
    //
    ...
    // --------------------------------------------------------------------------------
    // CRM segment store "save" callback.
    // --------------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------------
    // Normalize the record by setting undefined values to an empty string so the
    // form clears them out when we update it.
    ...
    // --------------------------------------------------------------------------------
    // Prepare the form for creating a new CRM segment based on the current one.
    // --------------------------------------------------------------------------------
    newBasedOn: function() {
        ...
        this.data = new REDfly.dialog.crmSegmentRecord({
            ...
            evidence_subtype_id: this.data.get('evidence_subtype_id'),
    ...
    updateDataRecordFromForm: function() {
        ...
        this.data.set('evidence_subtype_id', this.formElements.evidenceSubtype.getValue());
        ...
        Ext.each(
            ...
            function (item, index) {
                ...
                var stagingDataIdentifiersObject = {
                    ...
                    crm_segment_id: item.data.crm_segment_id,
    ...
    // --------------------------------------------------------------------------------
    // Set the REDfly identifier
    // @param id The REDfly identifier
    // --------------------------------------------------------------------------------
    ...
    setDataValue: function(
        ...
        switch (field)
        {
            ...
            case 'evidence_subtype_id':
                this.loadEvidenceSubtype(fieldData);
                this.data.set('evidence_subtype_id', fieldData);
                break;
            case 'fbtp':
