// --------------------------------------------------------------------------------
// The crmSegmentApprovalPanel is a stripped-down version of the crmSegmentPanel 
// used by the approveCrmSegment dialog and placed at its right side.
// --------------------------------------------------------------------------------
REDfly.component.crmSegmentApprovalPanel = Ext.extend(Ext.FormPanel, {
    ...
    initComponent: function() {
        ...
        var formElements = {
            ...
            evidenceSubtype: this.newFormField(
                'textfield',
        ...
        REDfly.component.crmSegmentApprovalPanel.superclass.initComponent.apply(this, arguments);
        var storeLoad = function(store, recordList, opt) {
            if ( recordList.length === 0 ) { return; }
            // Set the internal data record so we can access it later
            var crmSegmentRecord = recordList[0];
            this.data = crmSegmentRecord;
            if ( ! crmSegmentRecord.get('last_update') ) {
                crmSegmentRecord.set('last_update', 'N/A');
            }
            if ( crmSegmentRecord.get('date_added') === crmSegmentRecord.get('last_update') ) {
                crmSegmentRecord.set(
                    'curator',
                    crmSegmentRecord.get('curator_full_name') +
                        ' (Added On ' + crmSegmentRecord.get('date_added_formatted') + ')'
                );
            } else {
                crmSegmentRecord.set(
                    'curator',
                    crmSegmentRecord.get('curator_full_name') +
                        ' (Last Updated On ' + crmSegmentRecord.get('last_update_formatted') + ')'
                );
            }
            if ( crmSegmentRecord.get('last_audit') === null ) {
                crmSegmentRecord.set(
                    'auditor',
                    'N/A'
                );
                ...
                crmSegmentRecord.set(
                    'auditor',
                    crmSegmentRecord.get('auditor_full_name') +
                        ' (Last Audited On ' + crmSegmentRecord.get('last_audit_formatted') + ')'
                );
            }
            // Load any form elements with names matching a field in the record
            this.getForm().loadRecord(crmSegmentRecord);
    ...
    // --------------------------------------------------------------------------------
    // Reset the current form and load any data specified by the REDfly identifier.
    // @param redflyId The new REDfly identifier to load
    // --------------------------------------------------------------------------------
    ...
    newFormField: function(
        ...
        // Add link if this is the Pubmed ID.
...
// --------------------------------------------------------------------------------
// CRM segment approval window.
// --------------------------------------------------------------------------------
REDfly.dialog.approveCrmSegment = Ext.extend(Ext.Window, {
    // Ext.Window configs
    autoScroll: true,
    closeAction: 'hide',
    modal: true,
    ...
    initComponent: function() {
        ...
        this.entityPanels = [];
        this.redflyIdList = [];
        var mergePanel = new REDfly.component.crmSegmentPanel({
            mode: 'approve'
        });
        ...
        Ext.apply(this, {
            ...
            // Since the height depends on the size of the viewport
        ...
        REDfly.dialog.approveCrmSegment.superclass.initComponent.apply(this, arguments);
    ...
    updateApprovalPanel: function(
        ...
        switch (dataField) {
            ...
            case 'evidence_term':
                ...
                break;
            case 'evidence_subtype_term':
                this.mergePanel.setDataValue('evidence_subtype_id', panel.getDataValue('evidence_subtype_id'));
                break;
            case 'gene_name':
    ...
    // --------------------------------------------------------------------------------
    // Initialize the crmSegmentApprovalPanels based on the REDfly identifiers 
    // selected.
    // For each REDflyId one crmSegmentApprovalPanel will be added to the  approval
    // window.
    ...
    setRedflyIds: function(redflyIdList) {
        ...
        this.entityPanels = [];
        this.redflyIdList = [];
        // Clear out any bits left in the crmSegmentPanel
        ...
        for ( var counter = 0;
            ...
            if ( redflyId.substr(0, 5) !== 'RFSEG') {
                continue;
            }
            // Be sure to pass the formId in on creation or it will be null
            // in initComponent()
            var entityPanel = new REDfly.component.crmSegmentApprovalPanel({
                panelId: counter,
                redflyId: redflyId,
                title: redflyId
            });
    ...
    reject: function() {
        ...
        var deleteItems = new Ext.form.Checkbox({
            fieldLabel: 'Mark for deletion pending CRM segment(s)',
            name: 'delete_items'
        });
                        ...
                        listeners: {
                            click: function() {
                                Ext.Ajax.request({
                                    ...
                                    url: REDfly.config.apiUrl + '/jsonstore/crmsegment/reject'
                                });
                            },
                            scope: this
                        },
