// ================================================================================
// Dialog for performing CRUD operations of staging data only for CRM segments.
// ================================================================================
...
// --------------------------------------------------------------------------------
REDfly.dialog.crmstsRecord = Ext.data.Record.create([
    {   name: 'action',
        defaultValue: 'cancel'  },
...
// --------------------------------------------------------------------------------
REDfly.component.crmstsPanel = Ext.extend(Ext.FormPanel, {
    ...
    initComponent: function() {
        this.crmstsStore = new Ext.data.JsonStore({
            ...
            proxy: new Ext.data.HttpProxy({
                api: {
                    create: {
                        method: 'POST',                        
                        url: REDfly.config.apiUrl + '/jsonstore/crmsegmenttriplestore/save'
                    },
                    // No destroy for this store but the specification can not be undefined!
                    destroy: {
                        url: REDfly.config.apiUrl + '/jsonstore/crmsegmenttriplestore/destroy'
                    },
                    read: {
                        method: 'GET',                        
                        url: REDfly.config.apiUrl + '/jsonstore/crmsegmenttriplestore/load'
                    },
                    update: {
                        method: 'POST',                        
                        url: REDfly.config.apiUrl + '/jsonstore/crmsegmenttriplestore/update'
                    }
                }
            }),
            ...
            fields: REDfly.dialog.crmstsRecord,
            idProperty: 'ts_id',
        ...
        this.citationCrmstsStore = new Ext.data.JsonStore({
            // Ext.data.JsonStore configs
            autoLoad: false,
        ...
        // Enables the 'stagingdata' event generated by this class, REDfly.component.crmstsPanel,
        // to be detected by its parent class, Ext.Window.
        this.enableBubble('stagingdata');
        this.data = new REDfly.dialog.crmstsRecord();
        ...
        this.data.set('crm_segment_id', this.crmsId);
                        ...
                        function() {
                            ...
                            if ( Ext.isEmpty(id) ) {
                                Ext.MessageBox.show({
                                    ...
                                    fn: function() {
                                        Ext.WindowMgr.get('crmstsCrudWindowId').getEl().setStyle('z-index', '90000');
                                    },
        ...
        REDfly.component.crmstsPanel.superclass.initComponent.apply(this, arguments);
    ...
    reset: function() {
        this.getForm().reset();
        this.crmstsStore.removeAll(true);
        // Do not fire the 'clear' event or it will trigger the 'destroy' REST call.
        this.citationCrmstsStore.removeAll(true);
        this.data = new REDfly.dialog.crmstsRecord({
            action: 'cancel',
    ...
    loadFormFromRecord: function(store, recordList, opt) {
        var crmstsRecord = recordList[0];
        this.data = crmstsRecord;
        // As long as any form item/element has the 'name' attribute defined
        // when using form submit().
        this.getForm().loadRecord(crmstsRecord);
        this.loadCitation(crmstsRecord.get('pubmed_id'));
        // Making the following comboboxes targeting the species identifier given
        // No longer necessary since the Drosophila ontology has to be accessible
        // for all species annotation according to the PI
        //this.formElements.stageOnComboBox.assayed_in_species_id = crmstsRecord.get('assayed_in_species_id');
        //this.formElements.stageOffComboBox.assayed_in_species_id = crmstsRecord.get('assayed_in_species_id');
        // Loading the following comboboxes with their identifiers assigned from the record.
        this.loadStageOnComboBox(crmstsRecord.get('stage_on_id'));
        this.loadStageOffComboBox(crmstsRecord.get('stage_off_id'));
        this.loadBiologicalProcessComboBox(crmstsRecord.get('biological_process_id'));
        this.loadSexComboBox(crmstsRecord.get('sex_id'));
        this.loadEctopicComboBox(crmstsRecord.get('ectopic_id'));
        this.loadEnhancerOrSilencerAttributeComboBox(crmstsRecord.get('enhancer_or_silencer_attribute_id'));        
        this.doLayout();
    },
    // --------------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------------
    // Handles exceptions generated by the CRM segment store.
    ...
    // --------------------------------------------------------------------------------
    // Sets the action that the user is taking on this panel.
